...
def test_nut_finder():
    ...
    # These fields partially exist in 2003 --> 2021
    for row in result:
        row.pop("COAST_TYPE", None)
        row.pop("MOUNT_TYPE", None)
        row.pop("URBN_TYPE", None)
        row.pop("NAME_LATN", None)

    # Note in 2021, the NUTS NAME changed from upper to title case
    for row in result:
        row["NUTS_NAME"] = row["NUTS_NAME"].upper()
